...
struct ImageManagerTests {
    ...
    func createTestImageDescription() -> ImageDescription {
        ...
        // Placeholder for now
        return ImageDescription(reference: "test:latest", description: "test:latest", digest: "sha256:test")
    }
    
    // MARK: - List Images Tests
    
    @Test("List images returns array", .enabled(if: ProcessInfo.processInfo.environment["CI"] == nil))
    ...
    @Test("List images filters infrastructure images", .enabled(if: ProcessInfo.processInfo.environment["CI"] == nil))
    func testListImagesFiltersInfra() async throws {
        ...
        let hasInfraImage = images.contains { image in
            image.reference.contains("buildkit") || image.reference.contains("init")
        }
    ...
    @Test("Pull image requires started system", .enabled(if: ProcessInfo.processInfo.environment["CI"] == nil))
    ...
    @Test("Save images to valid directory", .enabled(if: ProcessInfo.processInfo.environment["CI"] == nil))
    ...
    @Test("Save images with empty list succeeds", .enabled(if: ProcessInfo.processInfo.environment["CI"] == nil))
    ...
    func testLoadImagesNonExistentFile() async throws {
        let nonExistentFile = URL(fileURLWithPath: "/nonexistent/path/image.tar")
    ...
    @Test("Load images requires started system", .enabled(if: ProcessInfo.processInfo.environment["CI"] == nil))
    ...
    @Test("Delete empty image list succeeds", .enabled(if: ProcessInfo.processInfo.environment["CI"] == nil))
    ...
    @Test("Delete images filters infrastructure images", .enabled(if: ProcessInfo.processInfo.environment["CI"] == nil))
    ...
    func testKeyValueStringSpecialChars() {
        let result = ImageManager.keyValueString(key: "key-with-dash", value: "value_with_underscore")
