...
struct ContainerManagerTests {
    ...
    func testValidEntityName() async throws {
        ...
        let validNames = [
            "test", "test-container", "test_container", "test.container",
            "test123",
        ]
    ...
    @Test(
        "List containers returns empty array initially",
    ...
    @Test(
        "Get non-existent container throws error",
        .enabled(if: ProcessInfo.processInfo.environment["CI"] == nil)
    )
    func testGetNonExistentContainer() async throws {
        ...
        do {
            _ = try await ContainerManager.getContainer(
    ...
    func testGetContainerLogNonExistent() async throws {
        let log = try await ContainerManager.getContainerLog(
            "nonexistent",
            boot: false
        )
        #expect(log.isEmpty)
    ...
    @Test(
        "Stop empty container list succeeds",
        .enabled(if: ProcessInfo.processInfo.environment["CI"] == nil)
    )
    ...
    @Test(
        "Delete empty container list succeeds",
        .enabled(if: ProcessInfo.processInfo.environment["CI"] == nil)
    )
    ...
    @Test(
        "Delete running container without force fails",
        .enabled(if: ProcessInfo.processInfo.environment["CI"] == nil)
    )
