...
public enum Parser {
    ...
    public static func platform(from platformString: String) throws -> Platform {
    ...
    public static func mount(from spec: String) throws -> Filesystem {
        ...
        let options = parts.count > 2 ? parts[2].split(separator: ",").map(String.init) : []
        
        // Determine if this is a volume or bind mount
        if source.hasPrefix("/") || source.hasPrefix(".") || source.hasPrefix("~") {
            // Bind mount (virtiofs)
            return Filesystem(type: .virtiofs, source: source, destination: destination, options: options)
        } else {
            // Named volume
            return Filesystem.volume(name: source, format: "ext4", source: "", destination: destination, options: options)
        }
    }
    ...
    private static func parseNamedMount(_ spec: String) throws -> Filesystem {
        ...
        let destination = pairs["destination"] ?? pairs["dst"] ?? pairs["target"] ?? "/"
        let source = pairs["source"] ?? pairs["src"] ?? ""
        let typeStr = pairs["type"] ?? "volume"
        
        switch typeStr {
        case "volume":
            let name = source.isEmpty ? VolumeStorage.generateAnonymousVolumeName() : source
            return Filesystem.volume(name: name, format: "ext4", source: "", destination: destination, options: volumeOpts)
        case "bind":
            return Filesystem(type: .virtiofs, source: source, destination: destination, options: volumeOpts)
        case "tmpfs":
            return Filesystem(type: .tmpfs, source: "", destination: destination, options: volumeOpts)
        default:
            throw ParserError.invalidMountSpec("unsupported mount type: \(typeStr)")
        }
    }
    ...
    public static func port(from spec: String) throws -> PublishPort {
        ...
        case 1:
            ...
            return PublishPort(hostPort: containerStart, containerPort: containerStart, proto: proto, count: count)
        ...
        case 2:
            ...
            return PublishPort(hostPort: hostStart, containerPort: containerStart, proto: proto, count: count)
        ...
        case 3:
            ...
            let count = max(hostCount, containerCount)
            return PublishPort(hostAddress: hostAddress, hostPort: hostStart, containerPort: containerStart, proto: proto, count: count)
            
        default:
            throw ParserError.invalidPortSpec("invalid port specification: \(spec)")
        }
    }
    
    private static func parsePortRange(_ range: String) throws -> (start: UInt16, count: UInt16) {
        if range.contains("-") {
            ...
            guard bounds.count == 2,
                ...
                throw ParserError.invalidPortSpec("invalid port range: \(range)")
            }
    ...
    public static func memoryInBytes(from spec: String) throws -> UInt64 {
        ...
        guard let value = Double(numericPart), value > 0 else {
            throw ParserError.invalidMemorySpec("invalid numeric value in: \(spec)")
        }
        ...
        case "b", "":
            multiplier = 1
        default:
            throw ParserError.invalidMemorySpec("unknown memory suffix: \(suffix)")
        }
    ...
    public static func rlimit(from spec: String) throws -> ProcessConfiguration.Rlimit {
        let parts = spec.split(separator: "=", maxSplits: 1).map(String.init)
        guard parts.count == 2 else {
            throw ParserError.invalidRlimitSpec("invalid rlimit format: \(spec)")
        }
        ...
        guard let soft = UInt64(values[0]) else {
            throw ParserError.invalidRlimitSpec("invalid soft limit: \(values[0])")
        }
        
        let hard: UInt64
        if values.count > 1 {
            guard let h = UInt64(values[1]) else {
                throw ParserError.invalidRlimitSpec("invalid hard limit: \(values[1])")
            }
            hard = h
        } else {
            hard = soft
        ...
        return ProcessConfiguration.Rlimit(limit: limitType, soft: soft, hard: hard)
    }
}
