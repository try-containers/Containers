...
public struct Filesystem: Sendable, Codable {
    ...
    public enum FilesystemType: Sendable, Codable, Equatable {
        case block(format: String, cache: CacheMode = .auto, sync: SyncMode = .none)
        case volume(name: String, format: String, cache: CacheMode = .auto, sync: SyncMode = .none)
        case virtiofs
        case tmpfs
    }
    ...
    public init(type: FilesystemType, source: String, destination: String, options: [String]) {
        self.type = type
        self.source = source
    ...
    public static func block(format: String = "ext4", source: String, destination: String, cache: CacheMode = .auto, sync: SyncMode = .none, options: [String] = []) -> Filesystem {
        Filesystem(type: .block(format: format, cache: cache, sync: sync), source: source, destination: destination, options: options)
    }
    
    /// Create a volume-based filesystem mount.
    public static func volume(name: String, format: String, source: String, destination: String, options: [String] = []) -> Filesystem {
        Filesystem(type: .volume(name: name, format: format), source: source, destination: destination, options: options)
    }
    
    /// Create a virtiofs shared filesystem mount.
    public static func virtiofs(source: String, destination: String, options: [String] = []) -> Filesystem {
        Filesystem(type: .virtiofs, source: source, destination: destination, options: options)
    }
    
    /// Create a tmpfs temporary filesystem mount.
    public static func tmpfs(destination: String, options: [String] = []) -> Filesystem {
        Filesystem(type: .tmpfs, source: "", destination: destination, options: options)
    }
